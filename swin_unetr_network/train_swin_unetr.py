"""
Training script for Swin-UNETR fat fraction prediction model.

Based on pixel_level_network/train.py but uses SwinUNETRWrapper from
swin_unetr_network.model_swin_unetr.
"""
...
class Trainer:
    """Training manager for Swin-UNETR fat fraction prediction."""
    ...
    def _build_model(self) -> nn.Module:
        """Build model from config — forwards full model config dict."""
    ...
    def train_epoch(self, train_loader) -> dict:
        ...
        return {
            ...
            'mae': np.mean(epoch_metrics['mae']),
            'rmse': np.mean(epoch_metrics['rmse'])
        }
    ...
    def validate(self, val_loader) -> dict:
        ...
        return {
            ...
            'mae': np.mean(val_metrics['mae']),
            'rmse': np.mean(val_metrics['rmse']),
            'correlation': np.mean(val_metrics['correlation'])
        }
...
def main():
    parser = argparse.ArgumentParser(description='Train Swin-UNETR fat fraction prediction model')
    parser.add_argument('--config', type=str,
                        default='config/config_swin_unetr/train_config.yaml',
    ...
    config['output_dir'] = str(Path("outputs") / "swin_unetr_run" / timestamp)
    ...
    if fat_sampling_cfg.get('enabled', False):
        ...
        t0 = time.time()
        median_map = {}
        split_path = Path(data_dir) / fat_split_cfg.get('split_file', 'data_splits_fatassigned.json')
        ...
        print(f"Computed {len(weights)} weights in {time.time() - t0:.1f}s "
              f"(computed {computed} medians, reused {len(median_map)})")
        train_sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)
    else:
        print("Fat-aware sampling disabled.")

    # Create dataloaders — pass crop_to_mask settings from config
    train_loader, val_loader, test_loader = get_dataloaders(
        ...
        crop_to_mask=config['data'].get('crop_to_mask', False),
        crop_margin=config['data'].get('crop_margin', 5),
    )
